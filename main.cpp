...
#ifdef LOCAL
auto& operator<<(auto&, const pair<auto, auto>&);
auto& operator<<(auto& os, const auto& x) requires (!same_as<decltype(x), string>) {
  os << '{';
  string sep;
  for (const auto& v : x) {
    os << sep << v;
...
auto& operator<<(auto& os, const pair<auto, auto>& x) {
